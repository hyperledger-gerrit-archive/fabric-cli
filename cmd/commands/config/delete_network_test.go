...
var _ = Describe("DeleteNetworkCommand", func() {
	var (
		cmd      *cobra.Command
	...
	JustBeforeEach(func() {
		cmd = config.NewConfigDeleteNetworkCommand(settings)
	})
	...
	It("should create a delete network commmand", func() {
		Expect(cmd.Name()).To(Equal("delete-network"))
...
var _ = Describe("DeleteNetworkImplementation", func() {
	var (
		impl     *config.DeleteNetworkCommand
		err      error
		out      *bytes.Buffer
		settings *environment.Settings
	)
