...
func (s *Settings) parseEnvironment() {
	if v, ok := os.LookupEnv("FABCTL_HOME"); ok {
		s.Home = Home(v)
	}
	if v, ok := os.LookupEnv("FABCTL_DISABLE_PLUGINS"); ok && v == "true" {
...
func (s *Settings) setEnvironment() error {
	if err := os.Setenv("FABCTL_HOME", s.Home.String()); err != nil {
		return err
	}

	if err := os.Setenv("FABCTL_DISABLE_PLUGINS", fmt.Sprint(s.DisablePlugins)); err != nil {
		return err
	}
